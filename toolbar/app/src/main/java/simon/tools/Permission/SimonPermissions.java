 ...
 */
public final class SimonPermissions {
     ...
     * 私有化构造函数
     */
    private SimonPermissions(Activity activity) {
        mActivity = activity;
    }
     ...
     * 设置请求的对象
     */
    public static SimonPermissions with(Activity activity) {
        return new SimonPermissions(activity);
    }

    /**
     * 设置权限组
     */
    public SimonPermissions permission(String... permissions) {
     ...
     * 设置权限组
     */
    public SimonPermissions permission(List<String> permissions) {
        if (mPermissions == null) {
     ...
     * 被拒绝后继续申请，直到授权或者永久拒绝
     */
    public SimonPermissions constantRequest() {
        mConstant = true;
        return this;
    }
